...
const SolarOrder = () => {
    ...
    const generateReport = () => {
        const doc = new jsPDF();
        doc.setFontSize(18);
        doc.text("Solar Order History Report", 14, 20);
        doc.setFontSize(11);
        ...
        autoTable(doc, {
            ...
            startY: 55,
            theme: 'striped',
            headStyles: { fillColor: [241, 196, 15] } // Solar Yellow Color
        });

        doc.save(`Solar_Order_Report_${user.companyName}.pdf`);
    };

  // SolarOrder.js user data fetching with useEffect
useEffect(() => {
        ...
        try {
            ...
            if (userEmail) {
                ...
                const [profileResponse, ordersResponse] = await Promise.all([
                    ...
                    axios.get(`${API_BASE}/orders/user/${userEmail}/Solar-User`)
                ]);
...
// --- handleSubmit කොටස ---
    ...
    try {
        ...
        if (response.status === 201) {
            alert("✅ Your Solar Invoice successfully saved!");
            setInvoice(null);
            setInvoiceBase64(""); 
            
            // 👈 ප්ලාස්ටික් ඕඩර්ස් රීෆ්‍රෙෂ් කරන්න මෙතනත් 'Solar-User' දාන්න
            const ordersResponse = await axios.get(`${API_BASE}/orders/user/${user.email}/Solar-User`);
            setOrders(ordersResponse.data);
    ...
    return (
        <div style={styles.container}>
            ...
            <style>
                ...
                .upload-area:hover { border-color: #f1c40f !important; background: rgba(241, 196, 15, 0.1) !important; }
                .tab-button:hover { transform: translateY(-5px); border-color: #f1c40f !important; }
                `}
            </style>
            ...
            <div style={styles.topBar} className="top-bar">
                <div style={styles.logoArea}>
                    ...
                    <div>
                        ...
                        <div style={{...styles.subTitle, color: '#f1c40f', fontWeight: 'bold'}}>Solar Energy Division</div>
                    </div>
                </div>
            ...
            <div style={styles.tabGrid} className="tab-grid">
                {tabs.map((tab) => (
                    <div 
                        ...
                        style={{
                            ...styles.tabBtn,
                            borderColor: activeTab === tab ? '#f1c40f' : 'rgba(255, 255, 255, 0.1)',
                            background: activeTab === tab ? 'rgba(241, 196, 15, 0.2)' : 'rgba(255, 255, 255, 0.05)',
                        }}
                    ...
                    >
                        ...
                        <span style={{
                            color: activeTab === tab ? '#f1c40f' : '#fff',
                            fontWeight: 'bold',
                            fontSize: '13px'
                        }}>
                            {tab}
                ...
                {activeTab !== 'VIEW ORDER DETAILS' ? (
                        ...
                        <div style={styles.qrGrid}>
                            <div style={styles.qrCard} className="qr-card">
                                ...
                                <div style={styles.infoRow}>
                                    ...
                                    <span style={{...styles.infoValue, color: '#f1c40f'}}>{user.role.toUpperCase()}</span>
                                </div>
                            </div>

                            <div style={styles.qrCard} className="qr-card">
                                <h3 style={styles.cardTitle}>Upload Solar Invoice</h3>
                        ...
                        <div style={{textAlign: 'center', marginTop: '30px'}}>
                            <div style={styles.qrPlaceholder}>
                                <div style={styles.qrBox}>
                                    {activeTab === 'ORDER QR' ? "🔳 Solar QR" : "☀️ Panel Order"}
                                </div>
                                <p style={{fontSize: '14px', color: '#f1c40f', marginTop: '10px', fontWeight: 'bold'}}>
                                    {activeTab === 'ORDER QR' ? "Request Solar QR Code" : "Request Solar Products"}
                                </p>
                            </div>
                             {invoice && (
                                <button style={{...styles.submitBtn, maxWidth: '400px'}} onClick={handleSubmit}>
                                    Submit Solar {activeTab}
                                </button>
                            )}
                        </div>
                    </>
                    ...
                    <div style={{ padding: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h3 style={styles.cardTitle}>Solar Order History</h3>
                            {orders.length > 0 && (
                                            ...
                                            <tr key={index} style={styles.tableRow}>
                                                <td style={styles.td}>
                                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                        <span style={{ fontWeight: '500' }}>{order.date || 'N/A'}</span>
                                                        <span style={{ fontSize: '11px', opacity: 0.7, color: '#f1c40f' }}>{order.time || ''}</span>
                                                    </div>
                                                </td>
        ...
        {order.status === 'QR Sent' && order.qrZipFile && (
            ...
            >
                 Download QR
                            ...
                            <p style={{ textAlign: 'center', opacity: 0.6, marginTop: '30px' }}>No solar order history found.</p>
                        )}
                    </div>
                )}
            </div>
...
const styles = {
    ...
    logoCircle: { width: '120px', height: '120px', background: '#fff', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', border: '3px solid #f1c40f' },
    logoImg: { width: '80%' },
    ...
    profileCircle: { width: '45px', height: '45px', background: '#222', borderRadius: '50%', border: '2px solid #f1c40f', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
    profileImg: { width: '100%', height: '100%', objectFit: 'cover' },
    profileLetter: { color: '#f1c40f', fontSize: '20px', fontWeight: 'bold' },
    ...
    qrBox: { fontSize: '32px', color: '#f1c40f', fontWeight: 'bold' },
    ...
    submitBtn: { width: '100%', maxWidth: '400px', padding: '12px', background: '#f1c40f', border: 'none', borderRadius: '10px', color: '#000', fontWeight: 'bold', cursor: 'pointer' },
    ...
    tableHeader: { borderBottom: '2px solid rgba(241, 196, 15, 0.5)', textAlign: 'left' },
    th: { padding: '12px', color: '#f1c40f', fontSize: '14px' },
    ...
    reportBtn: { background: '#f1c40f', color: '#000', border: 'none', padding: '10px 15px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' },
};

export default SolarOrder;